...
<?php while(have_posts()): the_post(); ?>
    <?php 
        ...
        $imagen_id  = get_post_meta($single_id, 'id_imagen_producto', true);
        $imagen_url = wp_get_attachment_image_url($imagen_id, 'large');
        $contact_url    = get_post_meta($single_id, 'contact_url', true);
        $message        = "Hello, I would like to know more about this product: " . get_permalink($single_id);
        $message        = urlencode($message);
    ?>
    <picture class="single__picture">
        <img src="<?= $imagen_url?>" alt="<?= get_the_title(); ?>">
    </picture>
    ...
    <div class="single__contact">
        <a href="<?= $contact_url . '?text=' . $message; ?>">
            Contact
